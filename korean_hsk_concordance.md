...
**입력:**
- `t12.dta` — HSK 변경 연계표 (HS_old, year_old, HS_new, year_new), 8,876행. 본 파일은 한국관세무역개발원이 자체 구축한 원본 연계표(`y0.dta`)를 양방향(순방향+역방향) 검증 절차를 거쳐 정정한 결과로, 원본에 존재하던 자기 매핑 잘못 기재 사례(살렸어야 하는데 죽인 경우 / 죽였어야 하는데 살린 경우)가 모두 교정된 상태이다.
